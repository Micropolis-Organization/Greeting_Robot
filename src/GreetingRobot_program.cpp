...
{
    int Sensor_Value = Sensor_One_Value.data;
    if (Sensor_Value == 1)
    {
...
void cmd_vel_cb(const geometry_msgs::Twist &cmdvel)
{
    ...
    if (Angle_Speed != 0.0)
    {
        ...
        Left_Wheel_Velocity = -1 * (Linear_Speed + Angle_Speed * (Center_Of_Rotation - LENGTH / 2));
        Right_Wheel_Velocity = -1 * (Linear_Speed + Angle_Speed * (Center_Of_Rotation + LENGTH / 2));
    ...
    // nh.loginfo("Received");
    odrive.SetVelocity(0, Right_Wheel_Velocity_In_RPS);
    odrive.SetVelocity(1, Left_Wheel_Velocity_In_RPS);
